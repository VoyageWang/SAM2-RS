...
class EvfSam2Model(PreTrainedModel):
    ...
    def initialize_evf_modules(self, config):
        ...
        # SAN 链接网络, 这个是需要训练的， 我们得到的 特征的维度是 bs, 128, 224, 224
        self.san_model = CNNResNet()
        for param in self.san_model.parameters():
                param.requires_grad = True    
        ...
        if self.config.san:
            # 获取原来的 Conv2d 层
            old_proj = self.mm_extractor.beit3.vision_embed.proj # 这里需要去看这个访问是否是有效的，到时候可以去打印下模型去看下

            # 重新定义 Conv2d 层，只修改 in_channels
            self.mm_extractor.beit3.vision_embed.proj = nn.Conv2d(
                in_channels=128,                      # 修改后的输入通道数
                out_channels=old_proj.out_channels,    # 保持原来的输出通道数
                kernel_size=old_proj.kernel_size,      # 保持原来的 kernel_size
                stride=old_proj.stride,                # 保持原来的 stride
                padding=old_proj.padding,              # 保持原来的 padding
                dilation=old_proj.dilation,            # 保持原来的 dilation
                groups=old_proj.groups,                # 保持原来的 groups
                bias=old_proj.bias is not None         # 保持原来的 bias 设置
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        backbone_out = self.visual_model.forward_image(images) # 这里的输出是sam的encoder的输出，应该是能够通过加一个adapter直接改的
        ...
        if self.config.san:
            # 需要去看是否是 896,896的shape
            assert images_evf.shape[2:] == (896, 896), "san input shape should be 896,896"
            images_evf = self.san_model(images_evf) 
