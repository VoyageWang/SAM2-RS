...
class HybridDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        rsris_seg_data = "rrsisd", # 这个是针对遥感的数据新加的
        model_type="ori",
        ...
        for dataset in self.datasets:
            ...
            elif dataset == "rsris_seg":
                from .rrsis_dataset import RRSISSegDataset
                self.all_datasets.append(
                    RRSISSegDataset(rsris_seg_data=rsris_seg_data, **kwargs)
                )
            elif dataset == "ade20k":
...
class ValDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        ds, splitBy, split = "rrsisd", "unc", "val"
        base_image_dir = base_image_dir
    ...
    def __getitem__(self, idx):
        ...
        print(image_path)
        print(sents)
        # 把这个image_path和sents保存到一个文件中
        # with open("val_data1.txt", "a") as f:
        #     f.write(image_path + "\n")
        #     for sent in sents:
        #         f.write(sent + "\n")
        # # 把这个文件闭合
